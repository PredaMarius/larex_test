...
export function* getMateriale(){
  ...
  try{
    ...
    if(egalitateIndicator){
      ...
      const materiale= yield strapi.getEntries('materials',{_limit:10000, _sort:'denumire:ASC'})
...
export function* getCurs(){
  ...
  try{
    ...
    const curs= yield fetch('https://corsproxy.io/?http://infovalutar.ro/bnr/azi/eur').then((rasp)=>rasp.text()).then((data)=>parseFloat(data))
